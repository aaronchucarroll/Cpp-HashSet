...
int main() {
  ...
  test("Test7", hs.contains(b),true); // add item m, but already exists there
  
  test("Test8", hs.contains(m), true); // should contain m
  test("Test9", hs.remove(m), true); // remove m
  test("Test10", hs.contains(m), false); // shouldn't contain m
  test("Test11", hs.remove(m), false); // can't remove again
